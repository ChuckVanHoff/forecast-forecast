''' Make a DataFrame of instants using pandas. '''
...
#     return pd.DataFrame.from_records([doc for doc in documents], index='_id')
...
#         # This next line converts the dicts to benedicts before flattening
#         instants = instants.append(row[1].apply(benedict.benedict.flatten))
...
def strip_keys(df):
    ''' Take a pandas.DataFrame and replace each dict value with the list of
    its values.
    '''
    ...
    for row in df.iterrows():
        ...
        for item in row[1]:
            if isinstance(item, dict):
                temp = [v for v in item.values()]
            elif isinstance(item, list):
                try:
                    temp = [v[1] for v in item]
...
def make_inst(df):
    ...
    for row in df.iterrows():
        ...
        for item in row[1]:
            if isinstance(item, dict):
                for v in item.values():
                    if isinstance(v, list):
                        item['weather'] = item['weather'][0]#v[0]
        # These next lines convert the dicts to benedicts before flattening,
        # sorting by keys, and then converting back to dicts.
        ...
        instants = instants.append(flat_sorted_data)
    np.save('instants.npy', instants)
    return instants
...
def make_deltas_df(df):
    ...
    return pd.concat(deltas, axis=1, ignore_index=False)
